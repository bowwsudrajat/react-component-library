...
import { RENAME_FILTER_MOMO, RENAME_FILTER_DEFAULT, getComponentName, handler } from './builder.mjs';
...
// const MOMO_ICONS_ROOT = path.join(currentDirectory, '../node_modules/material-design-icons/');
const MOMO_ICONS_ROOT = path.join(currentDirectory, './fixtures/material-design-icons/');
const MOMO_ICONS_SVG_DIR = path.join(MOMO_ICONS_ROOT, 'svg');
...
describe('builder', () => {
  ...
  it('should have icons to test with', () => {
    expect(fs.lstatSync(MOMO_ICONS_SVG_DIR).isDirectory()).to.equal(true);
  });

  describe('--output-dir', () => {
    const options = {
      svgDir: MOMO_ICONS_SVG_DIR,
      innerPath: '/svg/production/',
      glob: '/**/production/*_24px.svg',
      renameFilter: RENAME_FILTER_MOMO,
  ...
  describe('Template rendering', () => {
    ...
    it('should produce the expected output', async () => {
      ...
      cases.forEach((name) => {
        ...
        // fs.writeFileSync(path.join(MOMO_ICONS_ROOT, 'expected', name), actual, { encoding: 'utf8' });

        const expected = fs.readFileSync(path.join(MOMO_ICONS_ROOT, 'expected', name), {
          encoding: 'utf8',
        });
