...
const Accordion = React.forwardRef(function Accordion(inProps, ref) {
  const props = useDefaultProps({ props: inProps, name: 'MomoAccordion' });
  const {
...
Accordion.propTypes /* remove-proptypes */ = {
  ...
  children: chainPropTypes(PropTypes.node.isRequired, (props) => {
    ...
    if (isFragment(summary)) {
      return new Error(
        "MOMO: The Accordion doesn't accept a Fragment as a child. " +
          'Consider providing an array instead.',
      );
    }

    if (!React.isValidElement(summary)) {
      return new Error('MOMO: Expected the first child of Accordion to be a valid element.');
    }
