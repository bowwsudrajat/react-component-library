...
import useTimeout, { Timeout } from '@momo/utils/useTimeout';
import elementAcceptingRef from '@momo/utils/elementAcceptingRef';
import composeClasses from '@momo/utils/composeClasses';
import { alpha } from '@momo/system/colorManipulator';
import { useRtl } from '@momo/system/RtlProvider';
import appendOwnerState from '@momo/utils/appendOwnerState';
...
const Tooltip = React.forwardRef(function Tooltip(inProps, ref) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    childrenProps['data-momo-internal-clone-element'] = true;

    // eslint-disable-next-line react-hooks/rules-of-hooks
    React.useEffect(() => {
      if (childNode && !childNode.getAttribute('data-momo-internal-clone-element')) {
        console.error(
          [
            'MOMO: The `children` component of the Tooltip is not forwarding its props correctly.',
