...
const nestedFolder = {
  resolveId: (importee) => {
    ...
    if (importee.indexOf('@momo/utils/') === 0) {
      return resolveNestedImport('utils', importee);
    }

    if (importee.indexOf('@momo/private-theming/') === 0) {
      return resolveNestedImport('private-theming', importee);
    }

    if (importee.indexOf('@momo/styled-engine/') === 0) {
      return resolveNestedImport('styled-engine', importee);
    }

    if (importee.indexOf('@momo/system/') === 0) {
      return resolveNestedImport('system', importee);
    }
